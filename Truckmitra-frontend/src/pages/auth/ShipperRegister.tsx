...
const ShipperRegister: React.FC = () => {
        ...
        <React.Fragment key={i}>
          <div className={`flex flex-col items-center relative`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
              step >= i ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-gray-300 text-gray-500'
            }`}>
            ...
            <span className={`absolute -bottom-6 text-[10px] font-bold uppercase tracking-wider ${
              ...
              {i === 1 ? 'Business' : i === 2 ? 'Address' : 'Verification'}
            </span>
          </div>
          {i < 3 && (
            <div className={`w-16 h-0.5 mx-2 ${step > i ? 'bg-indigo-600' : 'bg-gray-200'}`} />
          )}
        </React.Fragment>
      ))}
    </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-4xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        {/* Left Side: Info */}
        <div className="md:w-1/3 bg-indigo-700 p-10 text-white flex flex-col justify-between">
          <div>
            <div className="text-3xl font-black tracking-tight mb-4 italic">TruckMitra</div>
            <h2 className="text-2xl font-bold mb-6 leading-tight">Empower Your Supply Chain</h2>
            <p className="text-indigo-100 text-sm leading-relaxed mb-8">
              Register as a Shipper to post loads, find reliable transporters, and track shipments in real-time.
            </p>
            <ul className="space-y-4">
              <li className="flex items-center text-sm">
                <div className="p-1 bg-indigo-600 rounded-full mr-3"><HiCheckCircle /></div>
                Access 50,000+ Verified Trucks
              </li>
              <li className="flex items-center text-sm">
                <div className="p-1 bg-indigo-600 rounded-full mr-3"><HiCheckCircle /></div>
                Lowest Market Quotes
              </li>
              <li className="flex items-center text-sm">
                <div className="p-1 bg-indigo-600 rounded-full mr-3"><HiCheckCircle /></div>
        ...
        <div className="md:w-2/3 p-10 md:p-14">
            ...
            {step === 1 && (
              <div className="space-y-6 animate-fadeIn">
                <div className="grid grid-cols-1 gap-6">
                  <InputField label="Contact Person Name" name="fullName" value={formData.fullName} onChange={handleChange} icon={<HiUser />} placeholder="Full Name" required />
                  <InputField label="Company Name" name="companyName" value={formData.companyName} onChange={handleChange} icon={<HiOfficeBuilding />} placeholder="Registered Company Name" required />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <InputField label="Email Address" type="email" name="email" value={formData.email} onChange={handleChange} icon={<HiMail />} placeholder="company@email.com" required />
                  </div>
                ...
                <div className="pt-6">
                  <button type="button" onClick={nextStep} className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg flex items-center justify-center">
                    Next: Office Address <HiArrowRight className="ml-2" />
                  </button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-6 animate-fadeIn">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2">
                    <InputField label="Full Office Address" name="address" value={formData.address} onChange={handleChange} icon={<HiLocationMarker />} placeholder="Building, Street, Landmark" required />
                  </div>
                ...
                <div className="flex space-x-4 pt-6">
                  ...
                  <button type="button" onClick={nextStep} className="flex-1 bg-indigo-600 text-white py-4 rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg flex items-center justify-center">
              ...
              <div className="space-y-8 animate-fadeIn">
                <div className="grid grid-cols-1 gap-6">
                  <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100">
                    <h3 className="text-indigo-800 font-bold mb-4 flex items-center"><HiIdentification className="mr-2" /> Identity & Taxes</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <div>
                        ...
                        <select
                          ...
                          className="block w-full px-4 py-3.5 bg-gray-50 border-2 border-gray-50 rounded-xl focus:bg-white focus:border-indigo-600 focus:outline-none transition-all text-gray-800 font-medium"
                          required
                        >
                      ...
                      <InputField label="Industry Type" name="industryType" value={formData.industryType} onChange={handleChange} placeholder="e.g. FMCG" required />
                    </div>
                    <div className="mt-4">
                ...
                <div className="flex space-x-4 pt-6">
                  ...
                  <button type="submit" disabled={isLoading || isUploading} className="flex-1 bg-green-600 text-white py-4 rounded-xl font-bold hover:bg-green-700 transition shadow-xl flex items-center justify-center disabled:opacity-50">
                    {isLoading ? 'Creating Account...' : 'Complete Registration'} <HiChevronRight className="ml-2" />
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="mt-10 text-center text-sm text-gray-500">
            Already registered? <Link to="/shipper/login" className="text-indigo-600 font-bold hover:underline">Sign In</Link>
          </div>
        </div>
      </div>
    </div>
  ...
  <div>
    ...
    <div className="relative group">
      ...
      <input
        ...
        onChange={onChange}
        {...rest}
        className={`block w-full ${icon ? 'pl-11' : 'pl-4'} ${rightIcon ? 'pr-12' : 'pr-4'} py-3.5 bg-gray-50 border-2 border-gray-50 rounded-xl focus:bg-white focus:border-indigo-600 focus:outline-none transition-all placeholder:text-gray-400 text-gray-800 font-medium`}
    ...
    <div className="flex flex-col">
      ...
      <button
        ...
        className={`h-24 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center transition-all ${
          value ? 'border-green-500 bg-green-50 text-green-600' : 'border-gray-200 bg-white text-gray-400 hover:border-indigo-400 hover:text-indigo-500'
        }`}
      >
        {value ? (
          <>
