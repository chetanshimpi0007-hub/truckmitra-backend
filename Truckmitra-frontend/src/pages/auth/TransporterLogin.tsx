...
const TransporterLogin: React.FC = () => {
  ...
  const handlePostLogin = (user: any) => {
    if (user.role !== 'TRANSPORTER') {
       toast.error('This login is for Transporters ONLY.');
       return;
    }
  ...
  return (
    <AuthLayout title="Transporter Login" subtitle="Enter your agency credentials">
      <form onSubmit={handleLogin} className="space-y-6">
        <div>
          ...
          <div className="relative group">
            ...
            <input
              ...
              placeholder="agency@email.com or 9876543210"
              required
            />
          </div>
        </div>
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-orange-600 text-white py-4 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-orange-700 transition shadow-xl shadow-orange-100 flex items-center justify-center disabled:opacity-50"
        >
          {isLoading ? 'Processing...' : 'Secure Login'} <HiArrowRight className="ml-2" />
        </button>
      </form>

      <div className="mt-10 text-center text-sm text-gray-500 font-medium">
        New to TruckMitra Transporters? <Link to="/transporter/register" className="text-orange-600 font-black hover:underline">Create Account</Link>
      </div>
    </AuthLayout>
  );
};
